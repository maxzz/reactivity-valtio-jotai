...
import { removeImm, swap, turnOffAutoComplete } from "@/utils";
...
function RowItemWithAtm({ item, name = 'dispname', ...rest }: { item: CatalogItem; name: StringRowKey; } & InputHTMLAttributes<HTMLInputElement>) {
    const snap = useSnapshot(item, { sync: true });
    const snapAtom = useState(atomWithProxy(item))[0];
    ...
    return (
        <input
            className="px-2 py-1 w-full text-xs text-primary-300 bg-primary-700 rounded" {...turnOffAutoComplete} {...rest}
            value={snapAtomAccess.dispname}
            onChange={(e) => { setSnapAtomAccess((v) => ({ ...v, [name]: e.target.value })); }}
        />
    );
}
...
export function Row({ item, idx, menuState }: RowParams) {
    ...
    return (
        <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-x-1">
            ...
            <RowItemWithAtm item={item} name="dispname" />
        ...
        <div className="text-xs grid gap-y-1">
            {snap.items.map((item, idx) => {
                const menuState: MenuState = {
                    // onDelete: (event: React.MouseEvent) => { event.preventDefault(); form.items = removeImm(items, idx); },
